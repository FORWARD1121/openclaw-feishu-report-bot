...
| 模块 | 状态 | 说明 |
|---|---|---|
| 周报生成主链路 | ✅ 已验证 | 示例 CSV 可生成 JSON 与趋势图 |
| 本地模板降级 | ✅ 已验证 | 无 API Key 时仍可生成总结 |
| 错误退出码 | ✅ 已验证 | 数据路径错误时返回非 0 |
| 健康检查 | ✅ 已验证 | 可检查依赖、配置、目录和端口绑定 |
| 飞书 Webhook 推送 | ⏳ 待真机验证 | 需要真实 Webhook 与签名密钥 |
| 飞书 WebSocket 追问 | ⏳ 待真机验证 | 需要真实飞书企业应用 |
| OpenClaw 自动调度 | ⏳ 待真机验证 | 需要 OpenClaw 运行环境 |
| Docker 一键启动 | ⏳ 待真机验证 | 需要目标机器 Docker 环境 |
